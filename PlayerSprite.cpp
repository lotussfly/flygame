...
bool CPlayerSprite::onTouchBegan(Touch *touch, Event *event)
{
...
void CPlayerSprite::onTouchMoved(Touch *touch, Event *event)
{
	//Touch* touch = (Touch*)(* touches.begin());
...
void CPlayerSprite::setShield()
{
	...
	cocos2d::Vector<CCSpriteFrame*> animFrames;
...
void CPlayerSprite::MissileExplore(CCNode *pSender)
{
	...
	cocos2d::Vector<cocos2d::SpriteFrame*> pAnimFrames;
	char str[64] = {0};
	for (int i = 1; i < 13; i++)
	{
		sprintf(str, "Missile_%02d.png", i); 
		CCSpriteFrame *frame = CCSpriteFrameCache::sharedSpriteFrameCache()->spriteFrameByName(str);
		pAnimFrames.pushBack(frame);
	}
